...
const allCategories: Category[] = ["kulture", "dashuri", "grate-shqiptare", "lifestyle", "argetim"];

export default function ArticleTags({ category, relatedCategories }: Props) {
  const categories = relatedCategories
    ? [category, ...relatedCategories.filter((c) => c !== category)]
    : [category, ...allCategories.filter((c) => c !== category).slice(0, 2)];
